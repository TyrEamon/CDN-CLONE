...
export default {
  async fetch(request, env, ctx) {
      ...
      if (subMatch) {
          return handleSubscriptionRequest(request, env, subMatch[1], subMatch[2], ctx);
      }

      const nodeListMatch = path.match(/^\/([a-zA-Z0-9]+)$/);
      if (nodeListMatch && !path.startsWith('/api/') && !path.startsWith('/admin') && path.length > 1) {
          return handleNodeListRequest(request, env, nodeListMatch[1], ctx);
      }
...
async function handleSubscriptionRequest(request, env, id, type, ctx) {
  queueAccessLog(env.WUYA, request, ctx, 'subscription_convert', { target: `${id}/${type}` });
  const proxySettings = await getProxySettings(env.WUYA);
...
if (!await getSetting(db, 'ACCESS_LOG_TOKEN')) {
    await setSetting(db, 'ACCESS_LOG_TOKEN', crypto.randomUUID());
}
...
function normalizeLogValue(value, maxLength = 512) {
...
function buildAccessLogEntry(request, eventType, extra = {}) {
  const url = new URL(request.url);
  return {
      ...
      method: normalizeLogValue(extra.method || request.method, 16),
      user_agent: normalizeLogValue(extra.user_agent || request.headers.get('User-Agent'), 512),
      target: normalizeLogValue(extra.target, 512)
  };
}
...
function queueAccessLog(db, request, ctx, eventType, extra = {}) {
  const task = insertAccessLog(db, buildAccessLogEntry(request, eventType, extra))
...
async function apiIngestAccessLog(request, env) {
  const db = env.WUYA;
  const expectedToken = await getSetting(db, 'ACCESS_LOG_TOKEN');
  const auth = request.headers.get('Authorization') || '';
  const suppliedToken = auth.startsWith('Bearer ')
      ? auth.slice(7).trim()
      : (request.headers.get('X-Access-Log-Token') || '');
  ...
  const entry = buildAccessLogEntry(request, body.event_type || 'snippet', body);
  await insertAccessLog(db, entry);
  return jsonResponse({ success: true });
}

async function apiGetAccessLogs(request, db) {
  const url = new URL(request.url);
...
async function handleApiRequest(request, env) {
  ...
  if (path === '/api/access_logs' && method === 'POST') return await apiIngestAccessLog(request, env);
...
async function apiGetSettings(request, db) {
  ...
  const combinedSettings = {
      ...
      ACCESS_LOG_TOKEN: settings.ACCESS_LOG_TOKEN || '',
...
async function getDashboardPage(domains, ipSources, settings) { 
  ...
  return `<aside class="sidebar">
      ...
      <nav class="sidebar-nav">
          ...
          <a href="#page-access-stats" class="nav-link" data-target="page-access-stats"><svg class="icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v18h18"></path><path d="M7 15l4-4 3 3 5-7"></path><path d="M18 7h1v1"></path></svg> <span>访问统计</span></a>
...
async function refreshAccessStats() {
  ...
  try {
      ...
      if (summary) summary.innerHTML = '<p class="status-failed">加载失败: ' + escapeHtml(e.message) + '</p>';
      showNotification('访问统计加载失败', 'error');
  }
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  navLinks.forEach(link => {
      link.addEventListener('click', (e) => {
          ...
          if (targetId === 'page-access-stats') refreshAccessStats();
          window.scrollTo(0, 0);
      });
  });
